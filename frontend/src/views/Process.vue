            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  문서를 업로드하면 LLM이 내용을 분석하여 여론 시뮬레이션에 적합한 온톨로지 구조(개체 유형 + 관계 유형)를 자동으로 생성합니다
                </div>
              </div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">생성된 개체 유형 ({{ projectData.ontology.entity_types?.length || 0 }})</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                ...
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    +{{ projectData.ontology.relation_types.length - 5 }} 개 관계 더...
                  </div>
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">인터페이스 설명</div>
                <div class="detail-content">
                  생성된 온톨로지를 기반으로 문서를 청크로 나눈 후 Zep API를 호출하여 지식 그래프를 구축하고 개체와 관계를 추출합니다
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                ...
                <div class="phase-api">다음 단계로 진행 준비</div>
              </div>
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = '업로드 대기 중인 파일이 없습니다. 홈으로 돌아가 다시 시도해 주세요'
    loading.value = false
    return
  }
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || '프로젝트 로딩 실패'
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = '프로젝트 로딩 실패: ' + (err.message || '알 수 없는 오류')
  } finally {
    loading.value = false
  }
}

...
class TimeSimulationConfig:
    """时间模拟配置（基于中国人作息习惯）"""
    # 模拟总时长（模拟小时数）
    total_simulation_hours: int = 504  # 默认模拟504小时（3周）
...
- The user base is Chinese, so the schedule should follow Beijing time habits
...
- Peak hours are 19–22 (activity coefficient 1.5)
- Activity declines after 23:00 (activity coefficient 0.5)
- General pattern: low activity at night, gradually increasing in the morning, moderate during work hours, peak in the evening
- **Important**: The example values below are for reference only. Adjust specific time slots based on the nature of the event and the characteristics of the participating groups.
  - For example: student group peak may be 21–23; media is active all day; official institutions only during work hours
...
Field descriptions:
- total_simulation_hours (int): Total simulation duration, 24–504 hours (up to 3 weeks); shorter for breaking events, longer for sustained topics
...
- peak_hours (int array): Peak activity hours, adjusted based on the event's participating groups
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format. The time configuration should reflect Chinese daily schedule habits."
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        ...
        return TimeSimulationConfig(
            total_simulation_hours=result.get("total_simulation_hours", 504),
...
For example: official announcements should be published by Official/University type, news by MediaOutlet, student opinions by Student.
...
- **Schedule follows Chinese daily habits**: almost no activity from 0–5 AM, most active during 19–22 in the evening
